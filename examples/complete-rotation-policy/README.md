...
| <a name="input_ibmcloud_target_account_api_key"></a> [ibmcloud\_target\_account\_api\_key](#input\_ibmcloud\_target\_account\_api\_key) | IBM Cloud API key for the target account. This key is required for cross-account setups to manage Secrets Manager and IAM resources. If not provided, the value of ibmcloud\_api\_key is used. Leave this unset when the service ID and Secrets Manager instance are in the same account. | `string` | `null` | no |
